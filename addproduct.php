...
if(isset($_POST["submit"]))
{
	...
	if($_FILES["addsong"]["error"]==4)
	{
		$song="no song";
	}
	else
	{
		if($_FILES["addsong"]["error"]==0)
		{
			$song=$_FILES["addsong"]["name"];
			$tname=$_FILES["addsong"]["tmp_name"];
			move_uploaded_file($tname,"songs/$song");	
		}
	}

$conn=mysqli_connect(host,user,pass,dbname) or die("Error in connection ". mysqli_connect_error());

$query="insert into addproduct(catid,subcatid,productname,product) values('$catid','$scid','$pname','$song')";
	...
	{
		$msg = "Song not added successfully";	
	}
}
        	...
        	<div id="new_released_section">
           	  ...
           	  <form action="" method="post" enctype="multipart/form-data" name="form1" id="form1">
           	    <table width="100%" height="280" align="center">
           	      <tr>
           	        <td width="26%" align="left"><strong class="sign"><span class="tabledata">Select Category</span></strong></td>
           	        <td width="74%" align="left"><label for="cat"></label>
	...
	else
	{
		while($x=mysqli_fetch_array($res))
		{
			...
			?>
       	            ...
       	            <input type="submit" name="viewsubcat" id="viewsubcat" value="viewsubcat" /></td>
       	          </tr>
           	      <tr>
           	        <td align="left" class="sign"><span class="tabledata">Select Sub Category</span></td>
           	        <td align="left"><label for="subcat"></label>
...
if(isset($_POST["viewsubcat"]))
{
	...
	$query="select * from addsubcat where catid='$cid'";
		$res=mysqli_query($conn,$query) or die("Error in query ". mysqli_error($conn));
		...
		else
		{
			...
			{
				print "<option value='$y[3]'>$y[1]</option>";		
			}
		}
}
			?>
           	      ...
           	      <tr>
           	        <td align="left" class="sign"><span class="tabledata">Song Name</span></td>
           	        <td align="left"><label for="productname"></label>
       	            <input type="text" name="productname" id="productname" /></td>
       	          </tr>
           	      <tr>
           	        <td align="left" class="sign"><span class="tabledata">Select Song</span></td>
           	        <td align="left"><label for="addsong"></label>
       	            <input type="file" name="addsong" id="addsong" /></td>
       	          </tr>
           	      <tr>
           	        <td class="sign">&nbsp;</td>
           	        <td align="left"><input type="submit" name="submit" id="submit" value="Add Song" /></td>
       	          </tr>
           	      <tr>
           	        <td class="sign">&nbsp;</td>
           	        <td align="left"><?php
